...
import { auditText, formatReport, rulesBrief, filterReport, detectDocumentProfile, hitFingerprint, diffAudit, serializeFingerprints, deserializeFingerprints, } from './rules.js';
...
function queueSave(stateFile, state) {
...
function isPaperFile(filePath, cwd) {
    ...
    if (PAPER_PATH_HINTS.some((h) => norm.includes(h.toLowerCase())))
        return true;
    if (cwd) {
...
export function apply(ctx, config = {}) {
    ...
    ctx.tools.register(defineTool({
        name: 'writing_rules',
        description: '返回论文写作纪律速查清单（dsh-plugin-writing-guard v0.5.1）：修改过程残留、主张校准、修辞模式、LLM 关联词、' +
    ...
    if (cfg.autoAuditOnWrite) {
        ...
        ctx.on('tools/post-execute', async (exec, _result, next) => {
            ...
            try {
                ...
                const turn = exec.turn ?? -1;
                const injState = injectCounts.get(agent.id);
                if (injState && injState.turn === turn && injState.count >= cfg.maxAutoInjectPerTurn)
                    return decision;
    ...
    if (cfg.autoBrief) {
        ...
        ctx.on('agent/turn-stopping', async ({ agent, turn }) => {
            try {
                ...
                if (!cwd || !/manuscript|paper|论文|稿件|修订|返修|review|审稿/.test(cwd))
                    return;
