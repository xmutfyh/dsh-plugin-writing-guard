...
import { auditDelivery, formatDeliveryReport, } from './delivery.js';
...
function onAgentLifecycle(ctx, event, handler) {
    const on = ctx.on;
 ...
 */
export function buildAutoAuditControl(target, diff) {
    const lines = [
    ...
    for (const h of diff.added) {
        lines.push(`finding rule=${h.ruleId}; severity=${h.severity}; kind=${h.findingKind ?? 'advisory'}; action=${h.action ?? 'KEEP'}; paragraph=${h.paragraphIndex}`);
    }
    lines.push('', 'Apply the minimum necessary edit. Prefer CUT over generating replacement prose.', 'REFRAME_TO_FACT means remove the defensive/reviewer-facing motive and retain only a fact independently supported by the manuscript or source material.', 'If an edit requires a scientific fact that is not in the authoritative material, QUERY rather than inventing it.', 'Do not add a sentence merely to explain why a concern has been addressed or why the authors should not be criticized.', '</writing_guard_control>');
    return lines.join('\n');
}
        ...
        try {
            ...
            return await new Promise((resolve, reject) => {
                execFile(pythonCmd, args, { maxBuffer: 10 * 1024 * 1024, timeout: 30_000 }, (err, stdout) => {
                    if (err)
                        reject(new Error(`DOCX text extraction failed: ${err.message}`));
                    else
                        resolve(stdout);
                });
            });
...
export function apply(ctx, config = {}) {
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Delivery Integrity 审计（CAL 检测）：检测工作上下文中的被否决方案、临时尝试、纠错过程是否泄漏到最终交付物。' +
        ...
        async execute(args) {
            ...
            if (typeof args.baseline === 'string' && args.baseline)
                opts.baseline = args.baseline;
            if (typeof args.finalState === 'string' && args.finalState)
                opts.finalState = args.finalState;
            if (Array.isArray(args.rejectedTerms)) {
                opts.rejectedTerms = args.rejectedTerms.filter((x) => typeof x === 'string');
            }
            if (Array.isArray(args.rejectedClaims)) {
                opts.rejectedClaims = args.rejectedClaims.filter((x) => typeof x === 'string');
            }
    ...
    ctx.tools.register(defineTool({
        name: 'writing_rules',
        description: '返回论文写作纪律速查清单（dsh-plugin-writing-guard v' + PLUGIN_VERSION + '）：control-context 隔离、论证经济性、过度解释、主张校准、科研完整性与确定性自查。' +
    ...
    async function callWordGuard(command, ...extraArgs) {
        const { execFile } = await import('node:child_process');
            ...
            try {
                const result = await new Promise((resolve, reject) => {
                    execFile(pythonCmd, args, { maxBuffer: 10 * 1024 * 1024, timeout: 60_000 }, (err, stdout, stderr) => {
                        if (err)
                            reject(new Error(`word_guard ${command} failed: ${err.message}\n${stderr}`));
                        else
                            resolve(stdout);
                    });
                });
                ...
                if (pythonCmd === candidates[candidates.length - 1])
                    throw e;
                // Try next candidate
            }
        }
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'DOCX 结构化扫描（dsh-plugin-writing-guard v' + PLUGIN_VERSION + '）：对 Word 文档执行结构化扫描，返回文档 profile、标题层级、段落信息、表格、复杂对象检测。' +
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Word 文档安全编辑（dsh-plugin-writing-guard v' + PLUGIN_VERSION + '）：对 .docx 文件执行局部安全编辑。' +
        ...
        async execute(args) {
            ...
            const scopeConfig = args.scopeConfig;
            const mode = args.mode ?? 'text_only';
            const outputPath = args.outputPath;
            if (!filePath)
                throw new Error('需要提供 filePath');
            if (!replacements || replacements.length === 0)
                throw new Error('需要提供 replacements');
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Word 文档写作审计（dsh-plugin-writing-guard v' + PLUGIN_VERSION + '）：对 .docx 文件执行写作纪律检查。' +
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Word 文档范围完整性检查（dsh-plugin-writing-guard v' + PLUGIN_VERSION + '）：验证编辑操作是否修改了请求范围之外的内容。' +
        ...
        isConcurrencySafe: () => true,
        async execute(args) {
            const fileBefore = args.fileBefore;
            const fileAfter = args.fileAfter;
            if (!fileBefore || !fileAfter)
                throw new Error('需要提供 fileBefore 和 fileAfter');
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Word 文档三线表格式化（dsh-plugin-writing-guard v' + PLUGIN_VERSION + '）：将 .docx 中所有表格转换为学术三线表格式。' +
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Word 文档公式审计（dsh-plugin-writing-guard v' + PLUGIN_VERSION + '）：对 .docx 中的原生 OMML 公式进行只读审计。' +
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'DOCX OOXML 包完整性检查（dsh-plugin-writing-guard v' + PLUGIN_VERSION + '）：解析全部 XML/relationships，' +
    ...
    ctx.tools.register(defineTool({
        ...
        description: '生成 DOCX 基准格式指纹（dsh-plugin-writing-guard v' + PLUGIN_VERSION + '）：记录 styles/settings/numbering 哈希、' +
