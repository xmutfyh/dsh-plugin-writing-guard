...
import { auditText, formatReport, rulesBrief } from './rules.js';
...
function isPaperFile(filePath, cwd) {
    ...
    if (cwd) {
        ...
        if (!rel.startsWith('..') && !path.isAbsolute(rel)) {
            ...
            if (firstSeg && [...PAPER_ROOT_DIRS].some((d) => d.toLowerCase() === firstSeg))
                return true;
        }
    }
    return false;
...
/** 按严重度过滤 hits，返回摘要文本 */
function summarizeHits(report, minSeverity) {
...
export function apply(ctx, config = Config) {
    ...
    ctx.tools.register(defineTool({
        name: 'writing_audit',
        description: '对论文/稿件文本执行写作纪律扫描（本地规则，零网络）：检测修改过程语句残留（revised/本轮/投稿前…）、' +
    ...
    ctx.tools.register(defineTool({
        name: 'writing_rules',
        description: '返回论文写作纪律速查清单（dsh-plugin-writing-guard）：修改过程残留、防御性写作、AI 痕迹句式（审稿人识别重点，' +
    ...
    if (cfg.autoAuditOnWrite) {
        ...
        ctx.on('tools/post-execute', async (exec, _result, next) => {
            ...
            try {
                ...
                if (state && state.turn === turn && state.count >= cfg.maxAutoInjectPerTurn)
                    return decision;
                // 只审计论文类文件
