...
function countRatherThan(text) {
    const matches = text.match(/\brather than\b/gi);
    return matches ? matches.length : 0;
}
function countColonTitles(text) {
...
function countLlTransition(text) {
...
function countCnConnectives(text) {
...
export function auditText(text, opts) {
    ...
    for (const rule of RULES) {
        ...
        if (rule.countThreshold !== undefined) {
            let count = 0;
            if (rule.id === 'em-dash-density')
                count = stats.emDashCount;
            else if (rule.id === 'rather-than-heavy')
                count = stats.ratherThanCount;
            else if (rule.id === 'colon-title')
                count = stats.colonTitleCount;
            else if (rule.id === 'llm-transition-overuse')
                count = stats.transitionCount;
            else if (rule.id === 'rule-of-three')
                count = stats.ruleOfThreeCount;
            else if (rule.id === 'cn-ai-connectives')
                count = stats.cnConnectivesCount;
