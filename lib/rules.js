...
function computeSectionProfile(text) {
    const sentences = splitSentences(text);
...
export function computeJournalProfile(text, opts) {
    ...
    const sectionProfiles = sections.map((sec) => {
    ...
    const citationDist = { count: 1, mean: globalCitationDensity, median: globalCitationDensity, p10: globalCitationDensity, p90: globalCitationDensity, std: 0 };
    return {
        metadata: {
...
function journalMetricScore(current, dist) {
...
export function auditJournalFit(text, profile) {
    ...
    for (const sec of sections) {
        ...
        const metrics = [];
        const addMetric = (metric, current, expected) => {
            const dist = typeof expected === 'object' ? expected : {
                count: 1,
                mean: expected,
                median: expected,
                p10: expected,
