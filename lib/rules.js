...
const RULES = [
    ...
    },
    {
        ...
        // v0.5.1：改名 + 文案降级（GPT：算法不做语义等价，不能声称"同一局限"）
...
// ---------------------------------------------------------------------------
/** v0.5.1：稳定指纹——aggregate（density/section）规则用 ruleId 即可（每文件每种最多一个），
 *  段落级用 ruleId + 规范化 snippet（去空白/截断；不含段落索引——编辑后行号会变） */
export function hitFingerprint(h) {
    // aggregate hit（density / section-based）：snippet 含 count/denominator 会随编辑变化，
    // 不能作为指纹（GPT P0：4/3200 → 4/3300 会被误判为 resolved+added）
...
/** v0.5.1：LaTeX 命令分类（GPT P1）——argument 是引用 key 的命令整体删除，不把 key 留进 prose */
...
export function preprocess(text) {
    ...
    while (i < lines.length) {
        ...
        if (/^\s*\$\$/.test(line) || /^\s*\\begin\{equation/.test(line)) {
            flushProse(buf);
            buf = [];
            // P0-1（GPT）：先检测单行闭合 $$...$$，避免把后续正文全部吞进 math
 ...
 * v0.5.1：维护 heading hierarchy——只有 top-level 章节（headingLevel 1 或未知）作为
 * canonical section，子标题（H2/H3、subsection）归属当前顶层章节
 * （GPT：Discussion 下的 "Sample size / External validity / Measurement" 不应被当成三个 section）。
 */
export function detectSections(view) {
    ...
    for (const seg of view.segments) {
        if (seg.kind === 'heading') {
            ...
            // 子标题（level>1）：不新开 section，正文继续归当前顶层章节
        }
...
export function rulesBrief() {
    return [
        '# 论文写作纪律速查（dsh-plugin-writing-guard v0.5.1）',
        '',
